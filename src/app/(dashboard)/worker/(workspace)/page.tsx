...
interface BurningTask {
  id: string; title: string; due_date: string; urgency: 'overdue' | 'today'
}
...
interface DashboardData {
  ...
  settings: { phone_mask_enabled: boolean }
}
...
function KpiCard({
  value, label, color, icon, glow,
}: {
  value: number | string; label: string
  color: string; icon: React.ReactNode; glow: string
}) {
  return (
    <div className={`relative overflow-hidden rounded-2xl p-4 bg-white/70 backdrop-blur-sm border border-white/60 shadow-lg ${glow}`}>
      <div className="flex items-start justify-between mb-3">
        ...
        <svg className="w-16 h-8 opacity-30" viewBox="0 0 64 32">
          <polyline
            points="0,28 12,20 24,22 36,10 48,14 64,4"
            fill="none" stroke="currentColor" strokeWidth="2"
            className={color.includes('indigo') ? 'text-indigo-600' : color.includes('emerald') ? 'text-emerald-600' : color.includes('red') ? 'text-red-500' : 'text-amber-500'}
          />
        </svg>
      </div>
...
function MiniDealCard({ deal, lang }: { deal: RedZoneDeal | NewLead; lang: string }) {
  ...
  const name = client ? `${client.first_name} ${client.last_name}`.trim() : (isHe ? 'לא מוגדר' : 'Неизвестно')
  ...
  return (
    <div className="flex items-center gap-3 p-3 rounded-xl bg-white/60 backdrop-blur-sm border border-white/50 hover:bg-white/80 transition-all">
      ...
      <div className="flex flex-col items-end gap-1 shrink-0">
        {deal.amount > 0 && (
          <span className="text-xs font-bold text-emerald-600">{fmt(deal.amount, deal.currency)}</span>
        )}
        {phone && (
          <a href={waLink(phone)} target="_blank" rel="noopener noreferrer"
            className="text-[10px] bg-emerald-100 text-emerald-700 px-2 py-0.5 rounded-full font-semibold hover:bg-emerald-200 transition-colors">
            WA
          </a>
        )}
      </div>
...
function GlassWidget({ title, children, action, className = '' }: {
    ...
    <div className="min-h-full p-4 lg:p-6 space-y-5" dir="rtl">
      ...
      <div className="flex items-start justify-between">
        ...
        <button
          ...
          disabled={loading}
          className="p-2.5 rounded-xl bg-white/70 backdrop-blur-sm border border-white/50 text-gray-400 hover:text-gray-600 shadow-sm transition-all disabled:opacity-40"
        >
      ...
      <div className="grid grid-cols-3 gap-3">
        <button onClick={() => setWaTemplateOpen(true)}
          className="flex flex-col items-center gap-2 py-4 rounded-2xl bg-gradient-to-br from-emerald-500 to-emerald-600 text-white shadow-lg shadow-emerald-200/60 hover:shadow-xl hover:shadow-emerald-200/80 transition-all active:scale-[0.97] text-xs font-bold">
        ...
        <button onClick={() => setNewLeadOpen(true)}
          className="flex flex-col items-center gap-2 py-4 rounded-2xl bg-gradient-to-br from-indigo-600 to-indigo-500 text-white shadow-lg shadow-indigo-200/60 hover:shadow-xl hover:shadow-indigo-200/80 transition-all active:scale-[0.97] text-xs font-bold">
      ...
      {/* ── Row: Red zone + Urgent tasks ───────────────────────────────── */}
      <div className="grid gap-4 lg:grid-cols-2">
        <GlassWidget
          title={isHe ? '🔴 אזור אדום' : '🔴 Красная зона'}
          action={
            data && data.red_zone_deals.length > 0
              ? <span className="text-[10px] font-bold bg-red-100 text-red-600 px-2 py-0.5 rounded-full">{data.red_zone_deals.length}</span>
              : null
          }
        >
          {!data ? (
            ...
            <p className="text-sm text-gray-400 py-3 text-center">💤 {isHe ? 'מחוץ לשעות עבודה' : 'Нерабочее время'}</p>
          ) : data.red_zone_deals.length === 0 ? (
            <p className="text-sm text-emerald-600 py-3 text-center font-medium">✅ {isHe ? 'הכל בסדר!' : 'Всё под контролем!'}</p>
          ) : (
            <div className="space-y-2">
              {data.red_zone_deals.map(d => <MiniDealCard key={d.id} deal={d} lang={language} />)}
            </div>
          )}
        </GlassWidget>
        ...
        <GlassWidget
          ...
          action={
            data
              ? <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${data.burning_tasks.length > 0 ? 'bg-amber-100 text-amber-600' : 'bg-gray-100 text-gray-400'}`}>
        ...
        >
          {!data ? (
            ...
            <p className="text-sm text-gray-400 py-3 text-center">{isHe ? 'אין משימות 🎉' : 'Нет задач 🎉'}</p>
          ) : (
            <ul className="space-y-1.5">
              {data.burning_tasks.map(t => (
      ...
      {data && data.new_leads.length > 0 && (
        ...
        >
          <div className="space-y-2">
            {data.new_leads.map(l => <MiniDealCard key={l.id} deal={l} lang={language} />)}
          </div>
        </GlassWidget>
      )}

      {/* ── Bottom row: KPI + Funnel + Commission ──────────────────────── */}
      <div className="grid gap-4 lg:grid-cols-3">
          ...
          {!data ? (
            ...
            <div className="space-y-2">
              <div className="flex items-baseline justify-between">
                ...
                {data.kpi.target_amount && (
                  <span className="text-[10px] text-gray-400">
                    {isHe ? `יעד: ${fmt(data.kpi.target_amount, data.kpi.currency)}` : `план: ${fmt(data.kpi.target_amount, data.kpi.currency)}`}
                  </span>
                )}
              </div>
              <div className="w-full bg-gray-100 rounded-full h-2.5">
                <div
                  className={`h-2.5 rounded-full transition-all duration-700 ${
                    (data.kpi.percent ?? 0) >= 100 ? 'bg-emerald-500' : (data.kpi.percent ?? 0) >= 60 ? 'bg-indigo-500' : 'bg-amber-400'
                  }`}
                  style={{ width: `${Math.min(data.kpi.percent ?? 0, 100)}%` }}
                />
              </div>
              <p className="text-[10px] text-gray-400">
                {monthName(data.kpi.period.month, language)} — {data.kpi.percent ?? 0}%
              </p>
            </div>
          )}
        </GlassWidget>
        ...
        {/* Funnel mini map */}
        <GlassWidget
          title={isHe ? '📈 מפת הפייפליין' : '📈 Карта пайплайна'}
          action={
            <Link href="/worker/pipeline" className="text-[10px] text-indigo-500 font-semibold hover:text-indigo-700">
                ...
                return (
                  <div key={s.name} className="flex-1 flex flex-col items-center gap-1">
                    ...
                    <div
                      ...
                      style={{ height: `${Math.max(6, Math.round((s.count / max) * 36))}px`, backgroundColor: s.color }}
                    />
      ...
      <div className="rounded-2xl bg-gradient-to-r from-amber-50 via-orange-50 to-yellow-50 border border-amber-100/80 p-4 flex items-center gap-4">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-amber-600 mt-0.5">
            {isHe
              ? 'המכירות שלך הן הצמיחה שלנו. בהצלחה היום!'
              : 'Твои продажи — наш рост. Удачи сегодня!'}
          </p>
        </div>
        <div className="text-amber-300 text-2xl select-none">✨</div>
      </div>
