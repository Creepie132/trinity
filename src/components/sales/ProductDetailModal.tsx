...
import { ChevronRight, ZoomIn, X } from 'lucide-react'
  ...
  return (
    <>
      ...
      >
        ...
        {product.image_url ? (
          <div
            className="relative mb-4 rounded-xl overflow-hidden bg-gray-50 dark:bg-gray-800 group cursor-zoom-in"
        ...
        <div className="space-y-2 mb-6">
          <div className="flex items-baseline justify-between p-3 bg-indigo-50 dark:bg-indigo-900/20 rounded-lg">
            <span className="text-sm text-gray-500">מחיר יחידה</span>
          ...
          {[
            ...
            { label: 'נפח / גודל', value: product.unit_volume || product.unit },
            { label: 'במלאי', value: stock !== undefined ? `${stock} יח'` : undefined },
          ].filter(f => f.value).map(f => (
            <div key={f.label} className="flex justify-between items-center py-1.5 border-b border-gray-100 dark:border-gray-700">
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 py-2.5 bg-indigo-600 text-white rounded-lg font-semibold hover:bg-indigo-700 transition-colors text-sm"
          >
            ➕ הוסף לעסקה
          </button>
          <button
            onClick={onBack}
            className="flex-1 py-2.5 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 rounded-lg font-medium hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors text-sm"
          >
            ביטול
          </button>
        </div>
