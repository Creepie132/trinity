...
export interface PaymentReportData {
  ...
  methods: string[]     // выбранные способы оплаты (иврит)
...
export function buildPaymentReportHTML(d: PaymentReportData): string {
  ...
  const summaryRows = Object.entries(byMethod).map(([method, data]) => {
    ...
    return `
      <tr>
        <td style="padding:8px 12px">${methodBadge(method)}</td>
        <td style="padding:8px 12px;text-align:center;color:#555;font-size:13px">${data.count}</td>
        <td style="padding:8px 12px;text-align:left;font-weight:700;color:${color};font-size:13px">${fmt(data.total)}</td>
      </tr>`
  }).join('')

  const paymentRows = d.payments.map((p, i) => `
    <tr style="border-bottom:1px solid #edf0f4;${i % 2 === 1 ? 'background:#f8f9fb' : ''}">
      <td style="padding:9px 12px;color:#888;font-size:11px">${p.date}</td>
      <td style="padding:9px 12px;font-weight:600;color:#1B2A4A;font-size:12px">${p.clientName}</td>
      <td style="padding:9px 12px">${methodBadge(p.method)}</td>
      <td style="padding:9px 12px;color:#555;font-size:11px;max-width:160px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">${p.description || ''}</td>
      <td style="padding:9px 12px;font-weight:700;color:#1B2A4A;white-space:nowrap;text-align:left">${fmt(p.amount)}</td>
    </tr>`).join('')

  const footerContacts = [d.orgEmail, d.orgPhone].filter(Boolean).join(' · ')
...
<div style="background:#fff;width:794px;min-height:1123px;margin:0 auto;display:flex;flex-direction:column;font-family:'Heebo',Arial,sans-serif">
  ...
  <div style="background:linear-gradient(135deg,#0f1e3d 0%,#1B2A4A 60%,#243560 100%);padding:28px 36px 22px;display:flex;justify-content:space-between;align-items:center">
    <div>
      ${d.logoDataUri
        ? `<img src="${d.logoDataUri}" style="height:80px;width:auto;object-fit:contain;filter:brightness(1.2) contrast(0.9)" alt="Logo">`
        : `<div style="color:#fff;font-size:20px;font-weight:800">${d.orgName}</div>`
      }
    </div>
    <div style="text-align:left">
      ...
      <div style="display:inline-flex;align-items:center;background:rgba(212,170,80,.12);border:1px solid rgba(212,170,80,.4);border-radius:20px;padding:3px 10px;font-size:11px;color:#D4AA50;font-weight:600;margin-top:6px">
        📅 ${d.fromDate} — ${d.toDate}
      </div>
    </div>
  </div>
  ...
  <!-- INFO STRIP -->
  <div style="background:#f8f9fb;border-bottom:1px solid #e2e6ed;padding:14px 36px;display:flex;gap:36px">
  ...
  <div style="padding:20px 36px 0">
    ...
    <table style="width:280px;border-collapse:collapse;font-size:12px;background:#f8f9fb;border:1px solid #e2e6ed;border-radius:8px;overflow:hidden">
      <thead>
        <tr style="background:#1B2A4A;color:#fff">
          ...
          <th style="padding:8px 12px;text-align:left;font-size:10px">סכום</th>
        </tr>
      </thead>
      <tbody>${summaryRows}</tbody>
      <tfoot>
        <tr style="background:#1B2A4A">
          <td colspan="2" style="padding:10px 12px;color:#fff;font-weight:700;font-size:12px">סה"כ</td>
          <td style="padding:10px 12px;color:#D4AA50;font-weight:800;font-size:14px;text-align:left">${fmt(total)}</td>
        </tr>
      </tfoot>
    </table>
  </div>
    ...
    <table style="width:100%;border-collapse:collapse;font-size:12px">
      <thead>
        <tr style="background:#1B2A4A;color:#fff">
          ...
          <th style="padding:10px 12px;text-align:left;font-size:10px;border-radius:6px 0 0 6px">סכום</th>
        </tr>
      </thead>
  ...
  <!-- FOOTER -->
  <div style="margin-top:auto;background:#1B2A4A;padding:12px 36px;display:flex;justify-content:space-between;align-items:center">
