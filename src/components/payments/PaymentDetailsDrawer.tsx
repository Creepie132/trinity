...
export function PaymentDetailsDrawer({
  ...
  const L = {
    he: {
      method: 'אמצעי תשלום', type: 'סוג', tranzilaId: 'מזהה Tranzila',
      internalId: 'מזהה פנימי', sendWhatsapp: 'שלח קבלה ב-WhatsApp',
      downloadPdf: 'הורד PDF (Tranzila)', refund: 'בצע החזר',
      refundConfirm: 'אשר החזר?', refundSuccess: 'ההחזר בוצע', refundError: 'שגיאה בהחזר',
    ...
    ru: {
      ...
      internalId: 'Внутренний ID', sendWhatsapp: 'Отправить квитанцию WA',
      downloadPdf: 'Скачать PDF (Tranzila)', refund: 'Возврат',
      refundConfirm: 'Подтвердить возврат?', refundSuccess: 'Возврат выполнен', refundError: 'Ошибка возврата',
  ...
  if (mounted && isMobile) {
    const mobActions = [
      { icon: <MessageCircle size={13} />, label: l.sendWhatsapp, onClick: handleWhatsApp, variant: 'green' as const, disabled: sendingReceipt, loading: sendingReceipt, hidden: !clientPhone },
      { icon: <FileText size={13} />, label: l.downloadPdf, onClick: handleDownloadPdf, variant: 'blue' as const, disabled: downloadingPdf, loading: downloadingPdf, hidden: payment.status !== 'completed' },
      { icon: <RotateCcw size={13} />, label: l.refund, onClick: handleRefund, variant: 'danger' as const, disabled: refunding, loading: refunding, hidden: !(isOwner && payment.status === 'completed') },
    ...
    return (
      <TrinityMobDetailShell
        ...
        avatarContent={initials || <Receipt size={16} />}
        avatarBg={mc.gradient}
        actions={mobActions}
  ...
  const desktopContent = (
    <div style={{ padding: '20px 18px 24px' }} className="space-y-4">
      <div style={{ background: sc.bg, border: `1px solid ${sc.border}`, borderRadius: 16, padding: '20px', textAlign: 'center' }}>
        <div style={{ fontSize: 44, fontWeight: 900, color: sc.color, letterSpacing: '-2px', lineHeight: 1, marginBottom: 8 }}>₪{Number(payment.amount).toFixed(2)}</div>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '4px 14px', borderRadius: 20, background: `${sc.dot}20`, border: `1px solid ${sc.dot}40` }}>
          ...
          <span style={{ fontSize: 12, fontWeight: 700, color: sc.color }}>{statusLabel[payment.status] || payment.status}</span>
        </div>
      </div>
      <div className="grid grid-cols-2 gap-3">
        <div style={{ background: 'linear-gradient(135deg,#f8fafc,#f1f5f9)', border: '1px solid #e2e8f0', borderRadius: 14, padding: '12px 14px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 4 }}><CalendarDays size={15} color="#94a3b8" /><span style={{ fontSize: 13, fontWeight: 700, color: '#1e293b' }}>{formattedDate}</span></div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}><Clock size={13} color="#94a3b8" /><span style={{ fontSize: 12, color: '#64748b' }}>{formattedTime}</span></div>
        </div>
        <div style={{ background: mc.bg, border: `1px solid ${mc.border}`, borderRadius: 14, padding: '12px 14px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}><span style={{ color: mc.color }}>{mc.icon}</span><span style={{ fontSize: 14, fontWeight: 700, color: '#1e293b' }}>{methodLabel[method] || method}</span></div>
        </div>
      </div>
      ...
      {clientPhone && <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 12, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div style={{ display: 'flex', alignItems: 'center', gap: 6 }}><Phone size={13} color="#94a3b8" /><span style={{ fontSize: 11, color: '#94a3b8', fontWeight: 600 }}>{l.phone}</span></div><span style={{ fontSize: 13, fontWeight: 600, color: '#334155', fontFamily: 'monospace' }} dir="ltr">{clientPhone}</span></div>}
      ...
      {!loadingDetails && saleItems.length > 0 && (
        <div>
          ...
          <div style={{ border: '0.5px solid #e8edf4', borderRadius: 14, overflow: 'hidden', background: '#fff' }}>
            {saleItems.map((item: any, idx: number) => { const isProduct = !!item.product_id; const itemTotal = Number(item.total_price || item.quantity * item.unit_price); return (<div key={item.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '11px 14px', borderTop: idx > 0 ? '0.5px solid #f1f5f9' : 'none' }}><div style={{ width: 34, height: 34, borderRadius: 10, background: isProduct ? '#fef3c7' : '#ede9fe', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>{isProduct ? <Package size={15} color="#d97706" /> : <Wrench size={15} color="#7c3aed" />}</div><div style={{ flex: 1, minWidth: 0 }}><p style={{ fontSize: 13, fontWeight: 600, color: '#1e293b', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', margin: 0 }}>{item.product_name}</p><p style={{ fontSize: 11, color: '#94a3b8', margin: '2px 0 0' }}>{item.quantity} × ₪{Number(item.unit_price).toLocaleString()}</p></div><span style={{ fontSize: 14, fontWeight: 800, color: '#1e293b', flexShrink: 0 }}>₪{itemTotal.toLocaleString()}</span></div>) })}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '10px 14px', background: 'linear-gradient(90deg,#1e293b,#0f172a)', borderTop: '0.5px solid #e8edf4' }}><span style={{ fontSize: 11, fontWeight: 600, color: 'rgba(255,255,255,0.5)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>{l.total}</span><span style={{ fontSize: 18, fontWeight: 900, color: '#fff' }}>₪{Number(payment.amount).toLocaleString()}</span></div>
          </div>
        </div>
      )}
      {(saleNotes || payment.description) && <div style={{ background: '#fffbeb', border: '1px solid #fde68a', borderRadius: 12, padding: '10px 14px' }}><p style={{ fontSize: 9, fontWeight: 700, color: '#d97706', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 5 }}>{saleNotes ? l.notes : l.description}</p><p style={{ fontSize: 13, color: '#78350f', margin: 0, lineHeight: 1.5 }}>{saleNotes || payment.description}</p></div>}
      <div className="space-y-2">
        {tranzilaId && <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '8px 12px', display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600 }}>{l.tranzilaId}</span><span style={{ fontSize: 11, fontFamily: 'monospace', color: '#64748b' }}>{tranzilaId}</span></div>}
        {saleId && <div style={{ background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 10, padding: '8px 12px', display: 'flex', justifyContent: 'space-between' }}><span style={{ fontSize: 10, color: '#94a3b8', fontWeight: 600 }}>{l.saleId}</span><span style={{ fontSize: 11, fontFamily: 'monospace', color: '#64748b' }}>{saleId.slice(0, 16)}…</span></div>}
  ...
  return (
    <Modal open={isOpen} onClose={onClose} darkHeader showCloseButton={false} width="700px" dir={dir} contentClassName="!p-0">
      <TrinityModalShell open={isOpen} onClose={onClose} icon={<Receipt />}
        title={clientName} subtitle={formattedFull} dir={dir} sidebarExtra={desktopSidebar}>
        {desktopContent}
      </TrinityModalShell>
    </Modal>
  )
