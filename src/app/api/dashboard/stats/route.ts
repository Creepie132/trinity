...
export async function GET(request: NextRequest) {
  try {
    // ── Auth: Bearer (mobile) или cookie (web) ─────────────────────────────
    ...
    // ── Ownership check: нельзя запросить чужие данные ────────────────────
    // isAdmin (super_admin) может запрашивать любую org
    ...
    // 2. Visits this month vs previous month (direct org_id filter)
    ...
    // 3. Revenue this month vs previous month (direct org_id filter)
    ...
    // 4. Average check (direct org_id filter)
