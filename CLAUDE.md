...
1. **После каждой задачи автоматически выполняй:** `npm run build && git add -A && git commit -m "описание" && git push origin main`
2. **Не спрашивай подтверждения на git push** — делай автоматически
3. **Не добавляй `Co-Authored-By` в коммиты** — делай простые коммиты без подписей
4. **Никогда не пуши если build упал с ошибками** — сначала исправь, потом пушь
5. **Отвечай на русском языке** — это основной язык общения в проекте
