-- Import ALL clients for beautymania organization (FIXED)
-- org_id: 1e77c781-3848-4b16-a623-693de123c6bc

-- First, let's check existing clients to avoid duplicates
DO $$
DECLARE
  v_org_id UUID := '1e77c781-3848-4b16-a623-693de123c6bc';
  v_count INT := 0;
BEGIN
  -- Insert only if phone doesn't exist for this org
  ...
  SELECT * FROM (VALUES
    ...
    (v_org_id, 'לודמילה', 'פריז', '523942455', null::text, 'אשקלון הרב מרדכי אליהו 13', null::date, '6.02.24'),
    ...
    (v_org_id, 'נדיה', 'אנטונובא', '528311041', null::text, 'תל אביב דרך חיים בר לב 165/13', null::date, 'סופק לבית 8.02.24 חסר 2 קרם גוף: A ו-B'),
    ...
    (v_org_id, 'Татьяна', 'Левин', '542052952', null::text, 'בת ים המלאכה טל.', null::date, 'Оплачено 190 שек. Доставка с 9 до 16'),
    ...
    (v_org_id, 'נטליה', 'דולגליוב', '507165523', null::text, 'כרמיאל מישעול סייפן 20/2 2175102', null::date, 'К оплате наличными 240 שек'),
    (v_org_id, 'Люда', 'Париж', '523942455', null::text, 'Ашкелон הרב מורדכי אליהו 13', null::date, 'К оплате 240 שек наличными'),
    (v_org_id, 'Шуля', 'Примак', '528581098', null::text, 'Ашкелон эмек израэль 11 кв 5', null::date, 'Оплачено 120 שек'),
    (v_org_id, 'ויקטוריה', 'גוטמן', '542838293', null::text, 'קרית אתא יוספטל 7', null::date, 'ДВА набора оплачено оба с Луи Виттон. Оплачено 480 שек'),
    (v_org_id, 'Yulia', 'Ayusheev', '508185566', null::text, 'ק.מוצקין בן גוריון 29/71 מיקוד 2642426', null::date, 'Оплатила 240 שек'),
    (v_org_id, 'Марина', 'Брон', '504321949', null::text, 'Кирьят Моцкин צהייל 5', null::date, 'Оплатила 240 שек'),
    (v_org_id, 'Мила', 'Фридланд', '546444093', null::text, 'Тель-Авив Нахалат Ицхак 10', null::date, 'Позвонить перед отправкой'),
    (v_org_id, 'יאנה', 'ריאבוי', '526962257', null::text, 'חולון הנרייטה סולד 60', null::date, 'Оплатила 190 שек'),
    (v_org_id, 'Маша', 'Дашко', '544666691', null::text, 'מרכז גולברי סקארה לוגיסטי צריפין ניר צבי', null::date, 'Нир Цви Оплачено 240 שек'),
    (v_org_id, 'אוקסנה', 'יורובסקי', '526392735', null::text, 'אשקלון אלי כהן 37 אשקלון דירה 35', null::date, 'Оплачено 240 שек'),
    (v_org_id, 'Наталия', 'Литвинчук', '542438316', null::text, 'Рамат Ган Зеев Жаботински 43', null::date, 'Оплачено 190 שек'),
    (v_org_id, 'ולריה', 'פינקל', '523394522', null::text, 'נתניה אזוה"ת יקום לייד שפיים בניין A איטליה.', null::date, 'Позвонить перед доставкой Оплатила 120 שек'),
    ...
    (v_org_id, 'Kseniya', 'Chail', '528445019', null::text, 'רמלה מרדכי סעדיה 9 ב/27', null::date, 'Оплачено 190 שек'),
    (v_org_id, 'אירינה', 'סמירנוב', '546688812', null::text, 'כרמיאל חטיבת יפתח 9/64 2199274', null::date, 'Оплачено 240 שек'),
    ...
    (v_org_id, 'Элла', 'Мизинова', '538218058', null::text, 'Ашкелон Шая Агнон 23-5', null::date, 'Оплатила 240 שק. Кремы подарок на 8 марта'),
  ...
  WHERE NOT EXISTS (
    ...
    WHERE c.org_id = t.org_id AND c.phone = t.phone
  );

  GET DIAGNOSTICS v_count = ROW_COUNT;
  RAISE NOTICE 'Imported % new clients', v_count;
END $$;
