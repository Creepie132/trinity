...
// ─── Mobile: single-screen bottom sheet ──────────────────────────────────────
function CreateVisitMobile({ open, onClose, preselectedClientId, preselectedDate, preselectedTime, onVisitCreated }: {
  ...
  const [form, setForm] = useState({
    clientId: preselectedClientId || '', serviceId: '', service: '',
    date: preselectedDate ? (preselectedDate instanceof Date ? preselectedDate.toISOString().split('T')[0] : String(preselectedDate)) : getDefaultDate(),
  ...
  useEffect(() => {
    ...
    setForm(p => ({
      ...
      date: preselectedDate ? (preselectedDate instanceof Date ? preselectedDate.toISOString().split('T')[0] : String(preselectedDate)) : p.date,
  ...
  const handleClose = () => {
    onClose(); setIsAppt(false); setSelClient(null)
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/visits', { method:'POST', headers:{'Content-Type':'application/json'},
        body: JSON.stringify({ clientId: form.clientId, serviceId: form.serviceId, service: form.service,
          date: form.date, time: form.time, duration: isAppt ? null : form.duration,
          price: isAppt ? '0' : form.price, quantity: form.quantity, notes,
          event_type: isAppt ? 'meeting' : 'visit', meeting_link: isAppt ? (form.meeting_link || null) : null }) })
  ...
  const accentBg = 'var(--trinity-accent-bg, rgba(45,106,79,0.2))'
          ...
          <motion.div className="fixed bottom-0 left-0 right-0 flex flex-col"
            ...
            <div className="flex-1 overflow-y-auto px-4 py-3 space-y-4" style={{ touchAction:'pan-y' }}>
              ...
              <div className="flex gap-1 p-1 rounded-xl bg-muted">
                {[{v:false, label: isHe?'ביקור':'Визит', ico:<Scissors size={12}/>},{v:true, label: isHe?'פגישה':'Встреча', ico:<MapPin size={12}/>}].map(o=>(
                  <button key={String(o.v)} onClick={()=>setIsAppt(o.v)}
                    className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all"
                    style={{ background: isAppt===o.v ? 'white' : 'transparent', color: isAppt===o.v ? '#4f46e5' : 'var(--muted-foreground)', boxShadow: isAppt===o.v ? '0 1px 4px rgba(0,0,0,.12)' : 'none', border:'none', cursor:'pointer' }}>
                    {o.ico}{o.label}
                  </button>
                ))}
              </div>
            ...
            <div className="flex-shrink-0 flex gap-3 px-4 pb-6 pt-3 border-t border-border">
              ...
              <button onClick={handleSubmit} disabled={!canSubmit||submitting}
                ...
                style={{ flex:2, background: canSubmit&&!submitting ? accent : 'rgba(0,0,0,0.15)', border:'none', cursor: canSubmit&&!submitting?'pointer':'not-allowed', transition:'background .2s' }}>
...
function CreateVisitDesktop({ open, onOpenChange, preselectedClientId, preselectedDate, preselectedTime, onVisitCreated }: CreateVisitDialogProps) {
  ...
  const handleClose = () => {
    onOpenChange(false); setStep(1); setIsAppt(false); setSelClient(null)
    setForm({ clientId:'', serviceId:'', service:'', date:getDefaultDate(), time:getDefaultTime(), duration:60, price:'', quantity:1, notes:'', city:'', address:'', meeting_link:'' })
  }
        ...
        <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          {selClient && (
            <div className="flex items-center gap-2 p-3 bg-indigo-50 border border-indigo-100 rounded-xl">
              ...
              <div>
                <p className="text-sm font-semibold text-indigo-800">{selClient.first_name} {selClient.last_name}</p>
                {selClient.phone && <p className="text-xs text-indigo-500">{selClient.phone}</p>}
              </div>
            </div>
          )}
        </div>
        ...
        <div className="space-y-5 animate-in fade-in slide-in-from-right-4 duration-300">
          ...
          <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-2xl space-y-2.5">
            <p className="text-xs font-bold text-indigo-700 uppercase tracking-wide flex items-center gap-1.5">
              <CheckCircle2 className="w-3.5 h-3.5"/>{isAppt?(isHe?'סיכום פגישה':'Сводка встречи'):(isHe?'סיכום ביקור':'Сводка визита')}
            </p>
            <div className="space-y-1.5 text-sm text-indigo-800">
              ...
              {svcName && <div className="flex items-center gap-2"><Scissors className="w-3.5 h-3.5 text-indigo-400 shrink-0"/><span>{svcName}{!isAppt&&form.price?` — ₪${form.price}`:''}{form.quantity>1&&<span className="ml-1.5 px-1.5 py-0.5 bg-indigo-200 text-indigo-700 rounded text-xs font-bold">×{form.quantity}</span>}</span></div>}
...
// ─── Public export: routes mobile ↔ desktop ──────────────────────────────────
