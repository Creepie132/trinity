...
const NAV = [
  {
    ...
    label_he: 'לוח בקרה', label_ru: 'Дашборд', exact: true,
  },
  {
    ...
    label_he: 'פייפליין שלי', label_ru: 'Мой пайплайн', exact: false,
  },
  {
    href: '/clients',
    icon: (<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0z"/></svg>),
    label_he: 'לקוחות', label_ru: 'Клиенты', exact: false,
  },
  {
    href: '/diary',
    ...
    label_he: 'משימות וקביעות', label_ru: 'Задачи и встречи', exact: false,
  },
  {
    href: '/inbox',
    ...
    label_he: 'תקשורת', label_ru: 'Коммуникация', exact: false,
  },
  {
    href: '/analytics',
    ...
    label_he: 'דוחות', label_ru: 'Отчёты', exact: false,
  },
  {
    href: '#kb',
    icon: (<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"/></svg>),
    label_he: 'מאגר ידע', label_ru: 'База знаний', exact: false,
  },
]
        ...
        {NAV.slice(0, 4).map(item => {
          ...
          return (
            <Link key={item.href} href={item.href}
              className={`flex flex-col items-center gap-1 px-3 py-1.5 rounded-xl transition-all ${isActive ? 'text-indigo-600' : 'text-gray-400'}`}>
              <div className={isActive ? 'scale-110' : ''}>{item.icon}</div>
              <span className="text-[9px] font-semibold truncate">{isHe ? item.label_he : item.label_ru}</span>
...
export default function WorkerLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-slate-100 via-blue-50/30 to-indigo-50/20" dir="rtl">
      ...
      <aside className="hidden lg:flex flex-col w-64 shrink-0 h-full bg-white/70 backdrop-blur-2xl border-l border-white/50 shadow-xl z-20">
        <div className="px-5 py-5 border-b border-gray-100/80">
          <div className="flex items-center gap-3">
            ...
            <div className="ms-auto"><NotificationBell lang={language} /></div>
          </div>
        </div>
      ...
      <main className="flex-1 overflow-y-auto h-full pb-20 lg:pb-0">{children}</main>

      <MobileNav pathname={pathname} lang={language} />

      <NewLeadModal open={newLeadOpen} lang={language}
        onClose={() => setNewLeadOpen(false)} onCreated={() => setNewLeadOpen(false)} />
    </div>
  )
}
