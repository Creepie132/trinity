...
// ─── Slug generation ──────────────────────────────────────────────────────────
// Транслитерация кириллицы + иврита → латиница, затем slugify
...
function generateSlug(title: string): string {
  return title
    ...
    .map(c => CYR_MAP[c] ?? c)
    .join('')
    .replace(/[^\w\s-]/g, '')   // убираем не-ascii спецсимволы (иврит и т.д.)
    .replace(/[\s_]+/g, '-')    // пробелы → дефисы
    .replace(/-+/g, '-')        // схлопываем множественные дефисы
    .replace(/^-|-$/g, '')      // убираем дефисы по краям
    || `post-${Date.now()}`     // fallback если строка пустая (напр. чистый иврит)
}

// ─── i18n ─────────────────────────────────────────────────────────────────────
const I18N = {
  he: {
    pageTitle: 'ניהול בלוג — Beautymania',
    pageSub: 'מאמרים ופרסומים באתר beautymania.co.il',
    newPost: 'פוסט חדש',
    editPost: 'עריכת פוסט',
    published: 'פורסם',
    draft: 'טיוטה',
    noPosts: 'אין פוסטים עדיין',
    noPostsHint: 'לחץ "פוסט חדש" כדי להוסיף',
    titleLabel: 'כותרת',
    titlePlaceholder: 'כותרת המאמר...',
    slugLabel: 'Slug (URL)',
    coverLabel: 'תמונת כותרת (URL)',
    coverPlaceholder: 'https://...',
    excerptLabel: 'תקציר',
    excerptPlaceholder: 'תיאור קצר לתצוגה ברשימה...',
    contentLabel: 'תוכן המאמר',
    publishLabel: 'פרסם',
    cancel: 'ביטול',
    save: 'שמור',
    saving: 'שומר...',
    ...
    colDate: 'תאריך',
    colActions: 'פעולות',
    totalPosts: 'פוסטים סה"כ',
    publishedCount: 'מפורסמים',
  },
  ru: {
    pageTitle: 'Блог — Beautymania',
    pageSub: 'Статьи и публикации для beautymania.co.il',
    newPost: 'Новая статья',
    editPost: 'Редактировать',
    published: 'Опубликовано',
    draft: 'Черновик',
    noPosts: 'Статей пока нет',
    noPostsHint: 'Нажмите "Новая статья" чтобы добавить',
    ...
    excerptPlaceholder: 'Короткий анонс для списка статей...',
    contentLabel: 'Содержание статьи',
    ...
    <div className="flex flex-wrap items-center gap-0.5 p-2 border-b border-gray-200 bg-gray-50 rounded-t-lg">
      {tools.map(({ icon: Icon, action, active, title }) => (
        <button key={title} type="button" title={title} onMouseDown={e => { e.preventDefault(); action() }}
          className={cn(
            'p-1.5 rounded-md transition-colors text-gray-600 hover:bg-gray-200',
...
const EMPTY_FORM: PostForm = {
  title: '', slug: '', cover_image: '', excerpt: '', content_html: '', published: false,
}

function PostEditor({ open, post, onClose, s }: {
  open: boolean
  post: BlogPost | null    // null = новая статья
  onClose: () => void
  s: S
}) {
  ...
  }, [post?.id])   // пересоздаём редактор при смене статьи

  const up = (key: keyof PostForm, val: string | boolean) =>
    setForm(f => ({ ...f, [key]: val }))

  const handleTitleChange = (val: string) => {
    up('title', val)
    if (!slugTouched) up('slug', generateSlug(val))
  }

  const handleSlugChange = (val: string) => {
    setSlugTouched(true)
    up('slug', val.toLowerCase().replace(/[^a-z0-9-]/g, ''))
  }
      ...
      <DialogContent className="max-w-3xl p-0 gap-0 max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6 space-y-5">
          ...
          <div className="space-y-1.5">
            ...
            <Input autoFocus value={form.title} onChange={e => handleTitleChange(e.target.value)}
              placeholder={s.titlePlaceholder} className="h-11" />
          </div>
          ...
          <div className="space-y-1.5">
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs text-gray-400 bg-gray-100 px-2 py-2 rounded border border-gray-200 whitespace-nowrap">
                beautymania.co.il/blog/
              </span>
              <Input value={form.slug} onChange={e => handleSlugChange(e.target.value)}
                placeholder="my-post-slug" className="h-10 font-mono text-sm" dir="ltr" />
            </div>
          </div>
          ...
          <div className="space-y-1.5">
            <Label className="text-sm font-semibold flex items-center gap-1.5">
              <ImageIcon className="w-3.5 h-3.5 text-indigo-500" />{s.coverLabel}
            </Label>
            <Input value={form.cover_image} onChange={e => up('cover_image', e.target.value)}
              placeholder={s.coverPlaceholder} dir="ltr" className="h-10" />
            {form.cover_image && (
          ...
          <div className="space-y-1.5">
            ...
            <textarea value={form.excerpt} onChange={e => up('excerpt', e.target.value)}
              placeholder={s.excerptPlaceholder} rows={2}
          ...
          <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl border border-gray-200">
            <div>
              ...
              <p className="text-xs text-gray-400 mt-0.5">
                {form.published ? s.published : s.draft}
              </p>
            </div>
...
export default function AdminBlogPage() {
  ...
  const { mutate: togglePublish } = useMutation({
    mutationFn: async ({ id, published }: { id: string; published: boolean }) => {
      const { error } = await supabase
        .from('blog_posts')
        .update({ published, ...(published ? { published_at: new Date().toISOString() } : {}) })
        .eq('id', id)
      if (error) throw error
    },
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin-blog-posts'] }),
  ...
  const handleDelete = (p: BlogPost) => {
    if (confirm(`${s.deleteConfirm} "${p.title}"?`)) deletePost(p.id)
  }
      ...
      <div className="grid grid-cols-2 gap-4 max-w-xs">
        <Card>
          <CardContent className="p-4">
            <p className="text-xs text-gray-500">{s.totalPosts}</p>
            <p className="text-2xl font-bold text-gray-800 mt-0.5">{posts.length}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
            <p className="text-xs text-gray-500">{s.publishedCount}</p>
                ...
                {posts.map(p => (
                  <div key={p.id} className="p-4 space-y-2">
                    <div className="flex items-start gap-3">
                      {p.cover_image && (
                        <img src={p.cover_image} alt="" className="w-14 h-10 object-cover rounded shrink-0 border" />
                      )}
                      ...
                      <Badge variant={p.published ? 'default' : 'secondary'} className={cn('shrink-0 text-xs', p.published ? 'bg-emerald-100 text-emerald-700' : '')}>
                        {p.published ? s.published : s.draft}
                      </Badge>
                    </div>
                    <div className="flex items-center gap-2">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center gap-2">
                            ...
                            <span className={cn('text-xs font-medium flex items-center gap-1', p.published ? 'text-emerald-600' : 'text-gray-400')}>
                              {p.published ? <><CheckCircle2 className="w-3 h-3" />{s.published}</> : <><Clock className="w-3 h-3" />{s.draft}</>}
                            </span>
                          </div>
                        </td>
