...
### 03.05.2026 — Employee Permissions ACL UI (коммит 34cdb51)

#### Что сделано

Полная переработка страницы `/settings/permissions` — из простого списка из 5 переключателей в полноценный ACL-редактор с пресетами.

**API (новые endpoints)**

`GET/PATCH /api/permissions/employee?userId=xxx`
- GET возвращает полную строку `staff_permissions` (все boolean-колонки)
- PATCH bulk-upsert всех разрешений одним запросом (`onConflict: org_id,user_id`)
- Whitelist 23 колонок — защита от injection через dynamic keys
- Owner-only, проверка через `auth.orgRole`

`GET/POST/DELETE /api/permissions/sets`
- GET — список пресетов org из `permission_sets`
- POST — создать пресет (name + permissions jsonb). При `is_default=true` сбрасывает флаг у остальных
- DELETE — удалить пресет по id с проверкой org_id

**UI `/settings/permissions`**

Карточки выбора сотрудников — визуально (grid, не select-dropdown). Аватар с инициалами, роль, чекмарк при выборе.

Пресеты разрешений — горизонтальный список pill-кнопок. Применение пресета мгновенно заполняет переключатели из `permissions jsonb`. Создание нового пресета из текущего состояния. Удаление (появляется при hover). Звёздочка у default-пресета.

Группы разрешений (5 групп, коллапсируемые):
- 👤 Клиенты (6 разрешений) — просмотр, редактирование, экспорт, удаление, ссылки, маскировка телефонов
- 📅 Визиты (3) — создание, просмотр чужих, удаление
- 💳 Финансы (6) — просмотр, приём, скидки, отмена, отчёты базовые/расширенные
- 📦 Склад (3) — перенос, добавление, удаление
- ⚙️ Настройки и сделки (5) — сделки, удаление сделок, услуги, управление сотрудниками, другие филиалы

Опасные разрешения (can_delete_*, can_manage_staff) — маркированы иконкой ⚠️ и цветом amber.

Progress bar активных разрешений + кнопки «Все» / «Сбросить».

Sticky save bar — появляется при наличии несохранённых изменений. Синяя полоска с именем сотрудника и кнопкой «Сохранить». Исчезает после сохранения.

Полностью двуязычный: иврит (RTL) + русский (LTR).

#### Затронутые файлы
- `src/app/(dashboard)/settings/permissions/page.tsx` (полная переработка)
- `src/app/api/permissions/employee/route.ts` (новый)
- `src/app/api/permissions/sets/route.ts` (новый)
