...
import { MessageCircle, Save, Loader2, Check, AlertCircle, CheckCircle2, Eye, EyeOff } from 'lucide-react'
...
export default function AdminWhatsAppPage() {
  ...
  async function loadData() {
    ...
    try {
      ...
      if (intData.integration) {
        ...
        setForm(f => ({
          ...f,
          providerType: intData.integration.provider_type,
          instanceId: intData.integration.instance_id,
        }))
      }
      if (statsRes.ok) {
  ...
  async function handleSave() {
    if (!form.instanceId) {
      toast.error(l ? 'חסר Instance ID' : 'Заполните Instance ID')
      return
    }
    if (!form.apiKey && !integration) {
      toast.error(l ? 'חסר טוקן API' : 'Заполните API токен')
      return
    }
    setSaving(true)
    try {
    ...
    <div className="space-y-6 pb-24">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">
            {l ? 'הגדרות WhatsApp' : 'Настройки WhatsApp'}
          </h1>
          <p className="text-sm text-slate-500">
            {l ? 'ניהול אינטגרציית WhatsApp ותור ההודעות' : 'Управление интеграцией WhatsApp и очередью сообщений'}
          </p>
        </div>
      </div>
      ...
      <div className={`flex items-center gap-3 px-5 py-4 rounded-2xl border ${
        ...
        <div>
          <p className={`font-semibold text-sm ${isConnected ? 'text-green-800' : 'text-amber-800'}`}>
            {isConnected
              ? (l ? '✅ WhatsApp מחובר ופעיל' : '✅ WhatsApp подключён и активен')
              : (l ? '⚠️ WhatsApp לא מוגדר' : '⚠️ WhatsApp не настроен')}
          </p>
          {isConnected && integration && (
            <p className="text-xs text-green-600 mt-0.5">
              {integration.provider_type} · {integration.instance_id} ·{' '}
              {l ? 'עודכן' : 'Обновлено'}: {new Date(integration.updated_at).toLocaleDateString()}
            </p>
          )}
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 space-y-5">
        <h2 className="font-bold text-slate-800 flex items-center gap-2">
          ...
          {l ? 'הגדרות ספק WhatsApp' : 'Настройки провайдера WhatsApp'}
        </h2>
        ...
        <div className="space-y-1.5">
          <label className="block text-xs font-semibold text-slate-500 uppercase tracking-wide">
            {l ? 'Instance ID / API URL' : 'Instance ID / API URL'}
          </label>
          <input type="text" value={form.instanceId}
          ...
          <p className="text-xs text-slate-400">
            {l ? 'לדוגמה: gate.whapi.cloud' : 'Например: gate.whapi.cloud'}
          </p>
        </div>
        ...
        <div className="space-y-1.5">
          ...
          <div className="relative">
            <input
              ...
              placeholder={integration ? (l ? 'השאר ריק לשמור את הנוכחי' : 'Оставь пустым чтобы не менять') : 'xagvIuqhg...'}
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 px-6 py-4 bg-white/90 backdrop-blur border-t border-slate-200 flex justify-end">
        <button onClick={handleSave} disabled={saving}
          className={`flex items-center gap-2 px-8 py-3 rounded-xl font-bold text-sm transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] shadow-md ${
            savedFlash ? 'bg-green-500 text-white' : 'bg-green-500 hover:bg-green-600 text-white'
          }`}>
          {saving ? <Loader2 size={16} className="animate-spin" /> : savedFlash ? <Check size={16} /> : <Save size={16} />}
          {saving ? (l ? 'שומר...' : 'Сохраняю...') : savedFlash ? (l ? 'נשמר!' : 'Сохранено!') : (l ? 'שמור הגדרות' : 'Сохранить настройки')}
        </button>
      </div>
    </div>
  )
