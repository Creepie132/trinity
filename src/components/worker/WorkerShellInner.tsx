...
type WorkerPrefetchTarget = 'worker-dashboard' | 'worker-pipeline' | 'worker-meetings'

async function prefetchWorkerModule(qc: QueryClient, module: WorkerPrefetchTarget) {
  const STALE = 30_000
  try {
    switch (module) {
      case 'worker-dashboard':
        await qc.prefetchQuery({
          ...
          queryFn: () => fetch('/api/worker/dashboard').then(r => r.json()),
          staleTime: STALE,
        })
        break
      case 'worker-pipeline':
        await qc.prefetchQuery({
          queryKey: ['worker-pipeline'],
          queryFn: () => fetch('/api/worker/pipeline').then(r => r.json()),
          staleTime: STALE,
        })
        break
      case 'worker-meetings':
        await qc.prefetchQuery({
          queryKey: ['worker-meetings', 'upcoming', ''],
          queryFn: () => fetch('/api/worker/meetings?filter=upcoming&search=').then(r => r.json()),
          staleTime: STALE,
        })
        break
    }
...
const WORKER_ADJACENT: Record<string, WorkerPrefetchTarget[]> = {
  '/worker/dashboard':  ['worker-pipeline', 'worker-meetings'],
  '/worker/pipeline':   ['worker-dashboard', 'worker-meetings'],
  '/worker/meetings':   ['worker-dashboard', 'worker-pipeline'],
}

function WorkerPrefetcher() {
  const qc = useQueryClient()
  const pathname = usePathname()
  const prefetchedRef = useRef<Set<string>>(new Set())

  useEffect(() => {
    const adjacent = WORKER_ADJACENT[pathname] ?? []
    adjacent.forEach(module => {
      if (!prefetchedRef.current.has(module)) {
        prefetchedRef.current.add(module)
        setTimeout(() => prefetchWorkerModule(qc, module), 200)
      }
    })
  }, [pathname, qc])
...
const WORKER_NAV = [
  {
    ...
    label_he: 'לוח בקרה', label_ru: 'Дашборд', workerOnly: true,
  ...
  {
    href: '/worker/presentations', exact: false,
    label_he: 'מצגות', label_ru: 'Презентации',
    icon: (<svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"/></svg>),
  },
  {
...
// ─── NavItem (desktop hover-prefetch) ────────────────────────────────────────

function NavItem({ item, isActive, isHe }: {
  item: typeof WORKER_NAV[0]
  isActive: boolean
  isHe: boolean
}) {
  const qc = useQueryClient()
  ...
  const onMouseEnter = useCallback(() => {
    timerRef.current = setTimeout(() => {
      const map: Record<string, WorkerPrefetchTarget> = {
        '/worker/dashboard': 'worker-dashboard',
        '/worker/pipeline':  'worker-pipeline',
        '/worker/meetings':  'worker-meetings',
      }
      const target = Object.entries(map).find(([p]) => item.href.startsWith(p))?.[1]
      if (target) prefetchWorkerModule(qc, target)
    }, 150)
  }, [item.href, qc])
...
// ─── MobileNavLink (touch prefetch) ──────────────────────────────────────────

function MobileNavLink({ item, isActive, isHe, onClose }: {
  item: typeof WORKER_NAV[0]
  isActive: boolean
  isHe: boolean
  onClose: () => void
...
// ─── WorkerShellInner — главный компонент ────────────────────────────────────

export function WorkerShellInner({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-gradient-to-br from-slate-100 via-blue-50/30 to-indigo-50/20" dir={dir}>
      ...
      {/* ── Desktop Sidebar ──────────────────────────────────────────────── */}
      <aside className="hidden lg:flex flex-col w-64 shrink-0 h-full bg-white/70 backdrop-blur-2xl border-e border-white/50 shadow-xl z-20">
        ...
        <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
          {navItems.map(item => {
            ...
            return <NavItem key={item.href} item={item} isActive={isActive} isHe={isHe} />
          })}
        </nav>
      ...
      <div className="flex flex-col flex-1 min-w-0 h-screen overflow-hidden lg:hidden">
        ...
        {/* Overlay */}
        {drawerOpen && (
          <div className="fixed inset-0 z-[60] bg-black/40 backdrop-blur-sm" onClick={() => setDrawerOpen(false)} />
        )}

        {/* Drawer Panel */}
        <div
          dir={dir}
          className={`fixed top-0 bottom-0 z-[70] w-72 flex flex-col bg-white/95 backdrop-blur-2xl shadow-2xl transition-transform duration-300 ease-in-out
            ${isHe ? 'right-0' : 'left-0'}
            ${drawerOpen ? 'translate-x-0' : isHe ? 'translate-x-full' : '-translate-x-full'}`}
        >
          <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100">
            ...
            <button onClick={() => setDrawerOpen(false)} className="p-2 rounded-xl hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-all">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          ...
          <nav className="flex-1 px-3 py-2 space-y-0.5 overflow-y-auto">
            {navItems.map(item => {
              const isActive = item.exact ? pathname === item.href : pathname.startsWith(item.href)
              return <MobileNavLink key={item.href} item={item} isActive={isActive} isHe={isHe} onClose={() => setDrawerOpen(false)} />
            })}
          </nav>
          ...
          <div className="px-4 pb-6 pt-2 border-t border-gray-100 space-y-2">
            ...
            <button onClick={handleLogout} className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium text-red-500 hover:bg-red-50 hover:text-red-600 transition-all">
              <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/>
              </svg>
              {isHe ? 'יציאה' : 'Выйти'}
      ...
      {showOnboarding && (
        <WorkerOnboarding onComplete={(lang) => { setLanguage(lang); setShowOnboarding(false) }} />
      )}
    </div>
  )
}
