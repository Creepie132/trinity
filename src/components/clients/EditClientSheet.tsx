...
export function EditClientSheet({ client, isOpen, onClose, onSaved, locale }: EditClientSheetProps) {
  ...
  async function handleSave() {
    ...
    // ещё не вернулся с сервера. В этом случае берём id из client напрямую
    // и показываем предупреждение только если id вообще отсутствует.
    if (!client?.id) {
      toast.error(
        locale === 'he'
          ? 'שגיאה: מזהה לקוח חסר'
          : 'Ошибка: не найден ID клиента',
        { duration: 4000 }
      )
      return
    }

    // Если id ещё optimistic — используем реальный id как только он появится.
    // Блокировать сохранение не нужно: API роут не принимает optimistic-* id
    // и вернёт ошибку, которую мы покажем пользователю через catch.
    setSaving(true)
    try {
