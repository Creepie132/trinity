...
const SOURCES = [
  ...
  { value: 'cold',      he: 'שיחה קרה',   ru: 'Холодный звונок' },
...
function DrawerContent({ dealId, lang, onClose, onUpdated }: {
  ...
  useEffect(() => { loadDeal(dealId) }, [dealId, loadDeal])
  ...
  return (
    <div dir={isHe ? 'rtl' : 'ltr'}
      className="fixed top-0 end-0 z-50 h-full w-full max-w-md bg-gradient-to-b from-slate-50 via-white to-blue-50/30
                 shadow-2xl border-s border-white/50 flex flex-col overflow-hidden
      ...
      <div className="shrink-0 relative overflow-hidden">
        <div className="bg-gradient-to-br from-indigo-600 via-indigo-600 to-purple-600 px-5 py-5 text-white">
          ...
          <div className="relative flex items-center justify-between mb-4">
              ...
              <div>
                {loading ? (
                  <div className="space-y-1.5">
                    <div className="h-4 w-32 bg-white/30 rounded-lg animate-pulse" />
                    <div className="h-3 w-24 bg-white/20 rounded-lg animate-pulse" />
                  </div>
                ) : (
                  <>
                    <p className="font-black text-base leading-tight">{clientName || (isHe ? 'לקוח' : 'Клиент')}</p>
                    {deal?.client?.phone && (
            ...
            <button onClick={onClose}
              className="w-9 h-9 rounded-2xl bg-white/20 hover:bg-white/30 flex items-center justify-center transition-all active:scale-90">
          ...
          {!loading && deal && (
            <div className="relative flex items-center gap-2 flex-wrap">
              ...
              {deal.amount > 0 && (
                <span className="text-[11px] font-bold px-3 py-1 rounded-full bg-emerald-400/40 text-emerald-100">
                  {fmt(deal.amount)}
                </span>
              )}
        ...
        {/* Auto-save indicator */}
        {(saving || saved) && (
          <div className={`px-5 py-2 flex items-center gap-2 text-xs font-semibold transition-all ${saving ? 'bg-amber-50 text-amber-600' : 'bg-emerald-50 text-emerald-600'}`}>
            {saving
              ? <><svg className="w-3 h-3 animate-spin" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"/><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"/></svg>{isHe ? 'שומר...' : 'Сохраняю...'}</>
              : <><svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7"/></svg>{isHe ? 'נשמר ✓' : 'Сохранено ✓'}</>
            }
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-6 space-y-3 pt-3">
        {loading && (
          <div className="space-y-3">
            {[1,2,3].map(i => (
              <div key={i} className="rounded-2xl bg-white/60 border border-white/50 p-4 space-y-3 animate-pulse" style={{ animationDelay: `${i*100}ms` }}>
