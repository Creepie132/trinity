...
export function ClientCardSettingsModal({
  ...
  function toggle(key: keyof ClientCardSettings) {
    const next = { ...local, [key]: !local[key] }
    setLocal(next)
    onSave(next) // сохраняем мгновенно
  }

  if (!isOpen) return null

  const items: { key: keyof ClientCardSettings; label: string; emoji: string }[] = [
              ...
              >
                <div
                  className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform duration-200 ${
                    local[key]
                      ? locale === 'he' ? 'translate-x-0.5' : 'translate-x-[1.375rem]'
                      : locale === 'he' ? 'translate-x-[1.375rem]' : 'translate-x-0.5'
                  }`}
                />
              </div>
            </button>
