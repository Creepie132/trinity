...
**Последнее обновление:** 2026-02-26 00:52 UTC  
**Версия:** 2.38.0

---

## 🔧 ОБНОВЛЕНИЯ v2.38.0 (2026-02-26) - Tailwind Card Utilities & Visual Hierarchy ✨

### ✅ Кастомные утилиты Tailwind для карточек (commits ebb7871, новый)

**Добавлены кастомные классы в `tailwind.config.js`:**
- `shadow-card` — мягкая тень для всех карточек (0 1px 3px rgba(0,0,0,0.05))
- `shadow-card-hover` — тень на ховер (0 4px 6px -1px rgba(0,0,0,0.1))
- `shadow-card-lg` — крупная тень для модалей
- `border-card` — светлый border (#f1f5f9)
- `bg-page` — фон страницы (#f8fafc)
- `rounded-card` — радиус карточек (16px)

**Глобальное правило стилизации:**
...
- Dashboard layout фон: `bg-page`
- Все виджеты дашборда: KpiCard, TodayVisitsWidget, TodayTasksWidget, RevenueChartWidget, IncomeExpensesWidget, QuickActionsPanel
- DashboardContent KpiCard с hover эффектом
